...
def PARSER(speech, PS={"1": 1, "2": 1, "3": 1, "4": 1}, return_history=False):
    ...
    while i < len(speech) - 1:  # while there is at least one stimulus left to process
        ...
        while i + n_components > len(
            speech
        ):  # if there is less stimulus left than attention capacity, then `decrease the capacity
            n_components -= 1
        ...
        for c in range(n_components):
            ...
            percept_history[i] = (
                component,
                comp_p,
            )  # components, and their occurrence probabilities
        ...
        for c in components:
            ...
            if (
                len(c) > 2
            ):  # find any part of c in any other element or part of element that is present in PS
                ...
                for part in c_parts:
                    for element in PS.keys():
                        for y in range(0, len(element)):  # parse the element by stimuli
                            if (
                                part == element[y : y + 1] and c != element
                            ):  # if any part of the element matches the part of the component (not taking into account the percept shaping unit itself)
                                PS[element] -= 0.00005
...
def c3_parser_learning():
    ...
    for ID in range(0, 50):  # across 30 runs
        ...
        ps, chunkrecord = PARSER(
            speech, PS={"1": 1, "2": 1, "3": 1, "4": 1}
...
def PARSER_c3_probability(training_seq):
    ...
    chunkrecord = PARSER(
        speech, PS={"1": 1, "2": 1, "3": 1, "4": 1}, return_history=True
    )  # with the rational chunk models, rational_chunk_all_info(seq, cg)
    p = []
    eps = 0.05
