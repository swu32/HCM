...
class Chunking_Graph:
    ...
    def update_transition_between_chunks(self, prev, current, dt):
        ...
        if prev in list(chunk_pair_f.keys()):
            ...
            if current in list(chunk_pair_f[prev][key_t].keys()):
                chunk_pair_f[prev][key_t][current] = (
    ...
    def forget(self):
        ...
        for item in list(chunk_f.keys()):
            ...
            if chunk_f[item] < self.deletion_threshold and item != self.zero:
                ...
                if item == ():
                    print(
                        "is an empty item in transition matrix key? ",
                ...
                for dt in list(chunk_pair_f[fromkey].keys()):
                    if chunk_pair_f[fromkey][dt] != {}:
                        for tokey in list(chunk_pair_f[fromkey][dt].keys()):
                            chunk_pair_f[fromkey][dt][tokey] = (
                                chunk_pair_f[fromkey][dt][tokey] * self.theta
                            )
                            if (
                                chunk_pair_f[fromkey][dt][tokey]
    ...
    def chunking_reorganization(self, prev, current, cat, dt):
        ...
        if (tuple(cat) not in list(chunk_f.keys())) & (
            tuple(current) in list(chunk_f.keys())
        ):
            ...
            if chunk_pair_f != {}:
                if tuple(current) in list(chunk_pair_f.keys()):
                    chunk_pair_f[tuple(cat)] = chunk_pair_f[
                        tuple(current)
                    ].copy()  # inherent the transition from the last chunk element
                for key in list(chunk_pair_f.keys()):
                    if str(dt) in list(chunk_pair_f[key].keys()):
                        if tuple(prev) in list(chunk_pair_f[key][str(dt)].keys()):
                            ...
                            chunk_pair_f[key][str(dt)][tuple(prev)] = (
                                chunk_pair_f[key][str(dt)][tuple(prev)] - 1
                            )
                            if (
                                chunk_pair_f[key][str(dt)][tuple(prev)] <= 0
                            ):  # delete the not used entries
        ...
        if (tuple(cat) in list(chunk_f.keys())) & (
            tuple(current) in list(chunk_f.keys())
        ):
                ...
                if tuple(prev) in list(chunk_pair_f.keys()):
                    if chunk_pair_f[tuple(prev)] != {}:
                        if str(dt) in list(chunk_pair_f[tuple(prev)].keys()):
                            if tuple(current) in list(
                                chunk_pair_f[tuple(prev)][str(dt)].keys()
                            ):
                                ...
                                if (
                                    ...
                                    chunk_pair_f[tuple(prev)][str(dt)].pop(
                                        tuple(current)
                                    )
        return
    ...
    def pop_transition_matrix(self, element):
        ...
        if transition_matrix != {}:
            ...
            for key in list(transition_matrix.keys()):
                if element in list(
                    transition_matrix[key].keys()
                ):  # also delete entry in transition matrix
    ...
    def get_transitional_p(self, prev, current):
        ...
        if (
            prev in list(self.M.keys()) and prev in list(self.T.keys())
        ) and current in list(self.T[prev].keys()):
            sum_transition = 0
