...
def learning_and_update_mtx(
    current_chunks, t, previous_chunk_boundary_record, chunk_termination_time, cg
):
    """
    ...
    for chunk in current_chunks:
        chunk_termination_time[
            np.array(chunk)[-1, :, :] > 0
        ] = t  # update chunk termination time for the current chunks
        ...
        for i in range(
            0,
            min(
                temporal_length_of_current_chunk + 1,
                len(previous_chunk_boundary_record),
            ),
        ):  # iterate through adjacency in
            candidate_adjacent_chunks = previous_chunk_boundary_record[
                -i - 1
            ]  # start from the closest
            # to the furtherest.
            for candidate_chunk in candidate_adjacent_chunks:
                combined_chunk, dt = check_adjacency_mtx(
                    candidate_chunk, chunk, t - i + 1, t + 1
                )
                if (
                    np.sum(np.abs(combined_chunk)) > 0
                    ...
                    cg, chunked = threshold_chunking_mtx(
                        candidate_chunk, chunk, combined_chunk, dt, cg
                    )
...
def learning_and_update(current_chunks_idx, chunk_record, cg, t, learn=True):
    ...
    if len(chunk_record) > 1:
        for chunk_idx in current_chunks_idx:
            ...
            while (
                delta_t <= temporal_length_chunk + n_t and len(chunk_record) > 1
            ):  # looking backward to find, padding length
                # adjacent chunks
                ...
                if chunkendtime in list(chunk_record.keys()):
                    ...
                    for prev_idx, _ in previous_chunks:
                        ...
                        if (
                            not samechunk and cg.chunks[prev_idx].variable == []
                        ):  # do not check variable chunks
                            combined_chunk, dt = check_adjacency(
                                prev_idx, chunk_idx, delta_t, t, cg
                            )
                            if combined_chunk is not None:
                                cg.chunks[prev_idx].update_transition(chunk_idx, dt)
                                if learn == True:  # online learning
                                    cg, chunked = threshold_chunking(
                                        prev_idx, chunk_idx, combined_chunk, dt, cg
                                    )
                delta_t = delta_t + 1
    return cg
...
def threshold_chunking_mtx(prev, current, combined_chunk, dt, cg):
    ...
    def hypothesis_test(cl, cr, Marg, T, dt, zero):
        ...
        N = np.sum(
            list(M.values())
        )  # still applicable to spatial and spatial temporal chunks?
        # Expected
        ep1p1 = M[cl] / N * M[cr]
        ep1p0 = M[cl] / N * (N - M[cr])
        ep0p1 = (N - M[cl]) / N * M[cr]
                ...
                if current in list(chunk_pair_f[prev][str(dt)].keys()):
                    if chunk_f[prev] > N:
                        if chunk_pair_f[prev][str(dt)][current] > N:
                            if (
                                hypothesis_test(
                                    prev, current, cg.M, cg.T, str(dt), cg.zero
                                )
                                == False
                            ):
        ...
        if current_idx in list(prev.adjacency[dt].keys()):
            if prev.count > N:
                if prev.adjacency[dt][current_idx] > N:
                    if (
                        hypothesis_test(prev_idx, current_idx, cg, dt) == False
                    ):  # reject null hypothesis
                        chunked = True
                        cg.chunking_reorganization(prev_idx, current_idx, cat, dt)
...
def check_overlap(array1, array2):
    ...
    for i0, i1 in itertools.product(
        np.arange(array1.shape[0]), np.arange(array2.shape[0])
    ):
...
def checkequal(chunk1, chunk2):
    if len(chunk1.content.intersection(chunk2.content)) == max(
        len(chunk1.content), len(chunk2.content)
    ):
        return True
    else:
        return False
...
def check_adjacency(prev_idx, post_idx, time_diff, t, cg):
    ...
    elif cg.chunks[prev_idx].conflict(
        cg.chunks[post_idx]
...
def check_adjacency_mtx(prev, post, end_point_prev, end_point_post):
    ...
    start_point_prev = (
        end_point_prev - temporal_len_prev
    )  # the exclusive temporal length
    ...
    # the stretching temporal length of the two chunks
    t_chunk = (
        max(end_point_post, end_point_prev)
        - min(end_point_post, end_point_prev)
        + delta_t
        + max(start_point_post, start_point_prev)
        - min(start_point_post, start_point_prev)
    )

    if (
        t_chunk == temporal_len_prev and t_chunk == temporal_len_post and prev == post
    ):  # do not chunk a chunk by itself.
        ...
        if adjacent == True:
            ...
            concat_chunk[
                concat_chunk_prev == concat_chunk_post
            ] = 0  # remove overlapping components
            concat_chunk[concat_chunk_post > 0] = concat_chunk_post[
                concat_chunk_post > 0
            ]
            return concat_chunk, dt
        else:
            concat_chunk = np.zeros(
                [t_chunk, W, H]
            )  # nonadjacent, then returns a high dimneisonal zero array
            return concat_chunk, dt
    ...
    for i in range(0, H):
        for j in range(0, W):
            ...
            if np.sum(np.abs(chunk)) > 0:  # not an empty chunks
                ...
                current_chunks.add(
                    matchingchunk
                )  # the chunk that ends right at the moment t
...
def possible_best_matching_chunks(cg, observation_to_explain):
    ...
    if len(cg.get_nonzeroM()) > 0:
        for (
            chunk
        ) in cg.get_nonzeroM():  # can also go from the biggest chunk to the smallest
            # this is checking whether and which chunk has started at the first observation of the termination time
            subchunk_match, match_size = check_subchunk_match_mtx(
                observation_to_explain, chunk, size=True
            )
            ...
            if subchunk_match:  # indicator that the subchunk is a match
                possible_matching_chunks.append(
                    chunk
                )  # can be that chunk ends earier or later than the current time point
                if (
                    np.array(chunk).shape[0] >= observation_len
                    and match_size >= best_match_size
                ):
...
def find_relevant_observations(seq, termination_time, t):
    ...
    for i in range(0, H):
        for j in range(0, W):
            ...
            relevant_observations[relevant_t:, i, j] = seq[
                termination_time[i, j] + 1 :, i, j
            ]
    relevant_observations, n_zero = refactor_observation(
        relevant_observations, return_max_k=True
    )  # cut such that
    # there is an event
    # happening
    # at the beginning.
...
def identify_chunks(cg, seq, termination_time, t, previous_chunk_boundary_record):
    ...
    """
    "Starting from the location where the last chunk terminates, " "needs explanation for what comes thereafter "
    ...
    if relevant_observations.shape[0] > 0:  # there are some observations to explain:
        ...
        observation_to_explain = (
            relevant_observations.copy()
        )  # the sequence the same size as chunks:
        if len(list(cg.M.keys())) > 0:
            ...
            current_chunks, termination_time, cg = identify_chunk_ending_with_M(
                observation_to_explain,
                cg,
                termination_time,
                t,
            ...
            termination_time = update_termination_time(
                current_chunks, termination_time, t_end_best_matching_subchunk
            )
            previous_chunk_boundary_record.append(current_chunks)
...
def identify_chunk_ending_with_M(
    observation_to_explain, cg, termination_time, t, previous_chunk_boundary_record
):
    current_chunks = set()
    debug = 1
    ...
    while np.sum(np.abs(observation_to_explain)) > 0:
        ...
        (
            possible_matching_chunks,
            best_matching_subchunk,
        ) = possible_best_matching_chunks(cg, observation_to_explain)
        if np.sum(np.abs(best_matching_subchunk)) <= 0:  # no currentmatching chunk
            # check for matching chunks at previous time points.
            if (
                possible_matching_chunks != []
            ):  # chunks end before the termination point
                max_chunk_size = 0
                ...
                delta_t = (
                    tuple_to_arr(observation_to_explain).shape[0]
                    - tuple_to_arr(best_matching_subchunk).shape[0]
                )
                t_end_best_matching_subchunk = t - delta_t

                previous_chunk_boundary_record[t_end_best_matching_subchunk].add(
                    best_matching_subchunk
                )

                cg.add_chunk_to_cg_class(best_matching_subchunk)
                # M = add_chunk_to_M(arr_to_tuple(best_matching_subchunk), M)
                termination_time, cg = learning_and_update_mtx(
                    {best_matching_subchunk},
        ...
        sz = min(
            len(observation_to_explain), len(best_matching_subchunk)
        ...
        if check_chunk_ending(best_matching_subchunk, observation_to_explain):
            current_chunks.add(
                arr_to_tuple(best_matching_subchunk)
            )  # best ending subchunk ends right at t
            cg.add_chunk_to_cg_class(arr_to_tuple(best_matching_subchunk))
            # M = add_chunk_to_M(arr_to_tuple(best_matching_subchunk), M)
        if debug >= 1000:
            exit()
        observation_to_explain, max_k = refactor_observation(
            observation_to_explain, return_max_k=True
        )
...
def identify_biggest_chunk(cg, seqc, checktype="full"):
    ...
    for chunk in reversed(cg.chunks):  # identify one chunk in sequence at one time...
        matchsize, match_len = chunkidentification(
            chunk, seqc
        )  # do the specific match by default
    ...
    return (
        maxchunk,
        seqc,
    )  # strange, maxchunk is indeed an instance inside cg, a chunk inside cg
...
def updatechunk(chunk, explainchunk, chunk_record, cg, max_chunk_idx, t):
    if chunk.abstraction == []:
        explainchunk.append(
            (chunk.index, int(chunk.T), list(cg.visible_chunk_list[chunk.index]))
        )
        cg.chunks[
            chunk.index
        ].update()  # update chunk count among the currently identified chunks
        chunk_record = updatechunkrecord(
            chunk_record, chunk.index, int(cg.chunks[chunk.index].T) + t, cg
        )
    ...
    while chunk.abstraction != []:
        for ck in chunk.abstraction:
            explainchunk.append(
                (ck.index, int(ck.T), list(cg.visible_chunk_list[ck.index]))
            )
            chunk_record = updatechunkrecord(
                chunk_record, ck.index, int(cg.chunks[ck.index].T) + t, cg
            )
            cg.chunks[
                ck.index
            ].update()  # update chunk count among the currently identified chunks
...
def identify_one_chunk(cg, seqc, explainchunk, chunk_record, t):  # _c_
    max_chunk_idx, seqc = identify_biggest_chunk(
        cg, seqc
    )  # identify and pop chunks in sequence
    explainchunk, cg, chunk_record = updatechunk(
        cg.chunks[max_chunk_idx], explainchunk, chunk_record, cg, max_chunk_idx, t
    )
...
def identify_one_chunk_approximate(cg, seqc, explainchunk, chunk_record, t):
    ...
    explainchunk.append(
        (
            max_chunk_idx,
            int(cg.chunks[max_chunk_idx].T),
            list(cg.visible_chunk_list[max_chunk_idx]),
        )
    )
    chunk_record = updatechunkrecord(
        chunk_record, max_chunk_idx, int(cg.chunks[max_chunk_idx].T) + t, cg
    )
    cg.chunks[
        max_chunk_idx
    ].update()  # update chunk count among the currently identified chunks
...
def updatechunkrecord(chunk_record, ckidx, endtime, cg, freq=True):
    if freq == True:
        if endtime not in list(chunk_record.keys()):
            ...
            if (
                ckidx not in chunk_record[endtime]
            ):  # one chunk can only be identified at one time point. when there are
...
def identify_latest_chunks(cg, seq, chunk_record, t):
    ...
    if no_observation:
        ...
        while seq_explained == False:
            ...
            cg, seqc, chunk_record, explainchunk = identify_one_chunk(
                cg, seqc, explainchunk, chunk_record, t
            )
        ...
        current_chunks_idx = [
            item[0] for item in explainchunk if item[1] == dt
        ]  # move at the pace of the smallest identified chunk
        seq = seqc
...
def refactor(seq, current_chunk_idx, cg):
    ...
    if seq != []:
        ...
        for item in seq:
            ...
            seqcopy.append(
                tuple(listobs)
            )  # there should not be negative ts, otherwise something is not explained properly
        return seqcopy
    else:
        return []
...
def check_subchunk_match_mtx(sequence_snapshot, chunk, size=False):
    ...
    if size:
        return (
            np.sum(np.abs(diff[chunk > 0])) == 0,
            np.sum(np.abs(chunk) > 0),
        )  # return also the explanation covered by
    # the chunk
    else:
        return (
            np.sum(np.abs(diff[chunk > 0])) == 0
        )  # location where the template matching matters
...
def rational_chunking(prev, current, combined_chunk, cg):
    ...
    if P_s_giv_s_last is not None:
        Expected_reward_cat = P_s_giv_s_last * len(current) + (1 - P_s_giv_s_last) * (
            -1
        ) * w * len(current)
...
def transition_into_probabilities(chunk_f, chunk_pair_f, prev, current):
    # decide if the number of exposure is significant enough to be concatinated together as a chunk
    if (
        prev in list(chunk_f.keys()) and prev in list(chunk_pair_f.keys())
    ) and current in list(chunk_pair_f[prev].keys()):
        sum_transition = 0
...
def hcm_learning(arayseq, cg, learn=True):
    ...
    while seq_over == False:
        ...
        current_chunks_idx, cg, dt, seq, chunk_record = identify_latest_chunks(
    ...
    while independence == False and Iter <= maxIter:
        ...
        while seq_over == False:
            ...
            cg = learning_and_update(
                current_chunks_idx, chunk_record, cg, Buffer.t, learn=False
            )
    ...
    for t in range(0, total_time):  # goes with the order of time.
        ...
        if np.sum(np.abs(sequence[t, :, :])) > 0:
            (
                ...
                t,
                previous_chunk_boundary_record,
            )  # chunks that ends right before t. the last

        else:
            cg.update_empty(
                1
            )  # update empty observation througout the iteration process
            current_chunks = set()  # no chunks ending at the current point
            previous_chunk_boundary_record.append(current_chunks)

        chunk_termination_time, cg = learning_and_update_mtx(
            current_chunks,
            t,
            previous_chunk_boundary_record,
            chunk_termination_time,
...
def rational_chunking_all_info(seq, cg, maxit=4):
    ...
    def hypothesis_test(cl, cr, pM, pT, N):
        ...
        chisquare = N * (
            p1p1 * ((op1p1 - p1p1) / p1p1) ** 2
            + p1p0 * ((op1p0 - p1p0) / p1p0) ** 2
            + p0p1 * ((op0p1 - p0p1) / p0p1) ** 2
        ...
        for cr in list(pT[cl].keys()):
            ...
            if hypothesis_test(cl, cr, pM, pT, N) == False:  # there is a correlation
                if np.sum(tuple_to_arr(cl)) > 0 and np.sum(tuple_to_arr(cr)) > 0:
                    clcr = arr_to_tuple(
                        np.concatenate((tuple_to_arr(cl), tuple_to_arr(cr)), axis=0)
                    )
                    proposed_chunk_p[clcr] = (cl, cr, p)
    sorted_proposal = {
        k: v for k, v in sorted(proposed_chunk_p.items(), key=lambda item: item[1][2])
    }

    # find the maximum transition probability, try chunking it
    it = 0  # number of iteration
    while (
        independence_test(pM, pT, N) == False and len(list(sorted_proposal.keys())) > 0
    ):
        ...
        for cl in list(new_pM.keys()):
            ...
            for cr in list(new_pT[cl].keys()):
                ...
                p = p_cl * cr_giv_cl
                if (
                    hypothesis_test(cl, cr, pM, pT, N) == False
        ...
        sorted_proposal = {
            k: v
            for k, v in sorted(proposed_chunk_p.items(), key=lambda item: item[1][2])
        }

        it = it + 1
        if (
            it > maxit or len(list(sorted_proposal.keys())) == 0
        ):  # in case there is no proposal, exit.
            break
...
def eval_pM_pT(bag_of_chunks, partitioned_sequence, freq=True):
    ...
    for chunk in bag_of_chunks:
        parsed_chunk = chunk
        if freq == True:
            M[chunk] = get_pM_from_partitioned_sequence(
                parsed_chunk, partitioned_sequence, freq=True
            )
        else:
            M[chunk] = get_pM_from_partitioned_sequence(
                parsed_chunk, partitioned_sequence, freq=False
            )
    for cl in bag_of_chunks:
        T[cl] = {}
        for cr in bag_of_chunks:
            T[cl][cr] = get_pT_from_partitioned_sequence(
                cl, cr, partitioned_sequence, freq=freq
            )
    return M, T
...
def get_pT_from_partitioned_sequence(cl, cr, partitioned_sequence, freq=True):
    ...
    if freq:
        ...
        if chunk1_count > 0:
            ...
            return (
                0.0  # returns 0 if there is no occurrance for the first probabilility
            )
...
def evaluate_KL_compared_to_ground_truth(reproduced_sequence, generative_marginals, cg):
    ...
    def kl_divergence(p, q):  # usually, p is the output, and q is the input.
        return sum(
            p[i] * np.log2(p[i] / q[i]) for i in range(len(p))
        )  # KL divergence in units of bits.
